...
def renamedict(net_dict,statedict):
    state_dict_new = dict()
...
def main():
    ...
    for epoch in range(args.epoch_start,args.num_epochs):
        ...
        for i, (images, target) in enumerate(dataloader_train):
            ...
            loss2 = criterion(output_sup1, target)
            loss3 = criterion(output_sup2, target)
            ...
            if step % args.validation_step == 0 :
                ...
                if precision > max_miou:
                    max_miou = precision
                    torch.save(net.module.state_dict(),os.path.join(cfgs.save_model_path, 'bs_voc_best3.pth'))
